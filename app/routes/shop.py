...
def anvil():
    ...
    with connect() as c:
        ...
        tier=roll_tier(); c.execute("UPDATE runs SET gold=gold-750 WHERE id=?",(rid,)); c.execute("INSERT INTO stat_anvil_offers(run_id,tier) VALUES (?,?)",(rid,tier))
    return jsonify(ok=True,offer={"tier":tier,"options":ANVIL[tier][1]})
